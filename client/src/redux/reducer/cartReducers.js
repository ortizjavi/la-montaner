...
const cartReducer = (state = CART_INITIAL_STATE, action) => {
    switch (action.type) {
      case actionTypes.ADD_CART_PRODUCT:
        ...
        const existItem = state.cartItems.find((product) => product.id === newProduct.id);
  
        if (existItem) {
          return { ...state, cartItems: state.cartItems.map((product) => product.id === existItem.id ? newProduct : product) };
        } else {
      ...
      case actionTypes.ORDER_STATUS: return {...state, orders: action.payload};
      case actionTypes.GET_ORDERS: return {...state, orders: action.payload};
