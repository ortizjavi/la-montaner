...
module.exports = {
  searchProducts: async (req, res, next) => {
    ...
    if (req.query.name) {
      //si se hace una búsqueda, se manda name por query
      try {
        ...
        const productLength = (await Product.find({ name: { $regex: regex } }))
          .length;
      ...
      //si no se hace búsqueda se mandan todos los productos(dependiendo ProductsPerPage)
...
const getPriceSort = async (req, res, next) => {
  ...
  try {
    ...
    const product = await Product.find({
      $and: [{ price: { $gte: prices[0] } }, { price: { $lte: prices[1] } }],
    })
      .limit(ProductsPerPage)
      .skip(ProductsPerPage * page)
      .sort({
